(function() {
  const apiUrlImparte = 'http://localhost:3000/imparte';
  ...
  form.onsubmit = e => {
    ...
    const url = editando
      ? `${apiUrlImparte}/${editIdP}/${editCodA}`
      : apiUrlImparte;
  ...
  window.eliminar = function(id_p, cod_a) {
    if (!confirm('¿Eliminar esta relación?')) return;

    fetch(`${apiUrlImparte}/${id_p}/${cod_a}`, { method: 'DELETE' })
      .then(res => res.json())
      .then(() => fetchImpartes());
  };
