...
export const update = (id: number, updatedProfesor: Profesor, callback: Function) => {
    const queryString = `
        UPDATE Profesor
...
export const updateById = (cod_p: number, profesor: Profesor, callback: Function) => {
  const queryString = 'UPDATE Profesor SET cod_p = ?, nom_p = ?, profesion_p = ?, dir_p = ?, tel_p = ?, WHERE cod_p = ?';
  
  db.query(queryString, [profesor.cod_p, profesor.nom_p, profesor.profesion_p,profesor.dir_p, profesor.tel_p, cod_p], (err, result) => {
    if (err) {
      return callback(err);
    }
